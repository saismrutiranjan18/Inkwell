...
export default function CoverPage() {
  ...
  useEffect(() => {
    async function load() {
      const [{ data: bookData, error: bookErr }, { data: chapterData }] = await Promise.all([
  ...
  async function handleExport() {
    ...
    try {
      ...
      setExportError('Export failed. Please allow popups and try again.')
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden">
      <nav className="bg-ink-900 px-6 flex items-center gap-4 flex-shrink-0" style={{ height: '52px' }}>
        <Link href={`/editor/${bookId}`} className="text-gold-500 hover:text-gold text-sm flex-shrink-0">
          ← Back to Editor
        </Link>
        ...
        <div className="flex items-center gap-2 ml-2">
          ...
          <button
            ...
            disabled={exporting}
            className="bg-gold text-ink-900 text-xs font-medium px-4 py-1.5 rounded-lg hover:bg-gold-300 disabled:opacity-60 transition-colors"
          >
        ...
        {view === 'cover' && (
          <>
            ...
            <div className="flex-1 bg-[#e8e0d0] overflow-y-auto flex flex-col items-center justify-center p-8">
              <CoverCard book={book} />
              <p className="text-ink-400 text-xs mt-4">Live preview — changes save automatically</p>
            </div>
          </>
        )}
        {view === 'preview' && (
...
function CoverCard({ book }: { book: Book }) {
  ...
  return (
    ...
    >
      ...
      {book.cover_subtitle && (
        <p className="text-[9px] tracking-[0.18em] uppercase opacity-60 mb-4">
          {book.cover_subtitle}
        </p>
      )}
      <div className="w-10 border-t mb-4" style={{ borderColor: theme.accent }} />
      <p className="text-sm opacity-75" style={{ fontStyle: 'italic', fontFamily: 'Lora, serif' }}>

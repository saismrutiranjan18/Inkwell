      ...
      <nav className="bg-ink-900 px-8 h-14 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <form action="/auth/signout" method="post">
            <button
              type="submit"
              className="text-xs text-gold-500 hover:text-gold border border-gold-700 px-3 py-1 rounded-md transition-colors"
            >
              Sign out
            </button>
          </form>
    ...
    <Link href={`/editor/${book.id}`} className="group block">
      ...
      >
        ...
        <div
          ...
          style={{
            borderTop: `1px solid ${theme?.accent ?? 'rgba(201,168,76,0.4)'}`,
            paddingTop: '8px',
            width: '40px',
          }}
        />
      ...
      <div>
        ...
        <p className="text-xs text-ink-400 mt-0.5">
          {(book.total_words ?? 0).toLocaleString()} words · {formatDate(book.updated_at)}
        </p>
      </div>
    </Link>
  )
